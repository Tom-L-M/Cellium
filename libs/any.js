...
/**
 * Retrieves the moment milisseconds na convert to a usable timestamp log.
 * @since 1.2.16
 * 
 * @return {String} 
 * 
 * @example <caption>  </caption>
 * getTimestampLog();   // ex:     00:33:14.920
*/
namespace.getTimestampLog = () => {
    let date = new Date();
    return date.toString().split(' ')[4] +'.'+ date.getMilliseconds().toString().padStart(3,'0');
}
 ...
 *  let progress = new any.ProgressBar(10);
 *  let s = setInterval(()=>{
...
*/
/**
 * First method implemented by the class. Updates the progress bar (moves one step right).
 * @since 1.2.17
 * @param  {Number} current The position to jump the progress to. Only used if you want a non-linear progress movement.
 * @return {undefined} 
*/
/**
 * Second method implemented by the class. Clears the progress bar.
 * @since 1.2.17
 * @return {undefined}
*/
